% This script is CAT-TEG infused transfer function parameter identification 
...
% Save Fitting Plots
save_fig = 0;
plot_path = "./Fig/TEG and CAT Infused Model/Sample Set F/";

% Save TEG Info Spreadsheet
save_table = 0;
table_name = "TEG_Info_Infused_Model_SetF.xlsx";
...
for ii = 1:length(crd_files)
    ...
    TEG_info_extend_temp = strings(size(TEG_info_initial, 1),14);       % Use this in the parfor loop
    TEG_info_extend = strings(size(TEG_info_initial, 1),13);
    ...
    opts = optimoptions('lsqcurvefit','OptimalityTolerance', 1e-12, ...
        'MaxFunctionEvaluations', 1e6, 'FunctionTolerance', 1e-12, ...
        'MaxIterations', 5000, 'StepTolerance', 1e-12, ...
    ...
    for i = 1:sample_num
        ...
        if ~contains(table2array(TEG_info_initial(TEG_info_idx, ["R(min)", "K(min)", "MA(mm)", "Angle(deg)",  "TMA(min)", "LY30(%)"])), "NA")
            ...
            estimated_tf_coeff = TEGFittingNewModel(time_min, value, Ly60, TMA, opts);
        else
            error_flag = 1;
            estimated_tf_coeff = -1*ones(11,1);
        end
        ...
        if error_flag == 0
            simTF = TEGModelTimeVaryingCATFitting(estimated_tf_coeff, time_min);
            ...
            legendStr = sprintf('Fitted Curve (R^2 = %.3f)\nKn = %.2e\nK0 = %.2e\nK1 = %.2e\nK2 = %.2e\nKd = %.2e\nKnc = %.2f\nKpc = %.2f\nKdc = %.2f\nKn2 = %.2f\nKp2 = %.2f\nKd2 = %.2f', ...
                ...
                estimated_tf_coeff(4), estimated_tf_coeff(5), estimated_tf_coeff(6), estimated_tf_coeff(7), estimated_tf_coeff(8), estimated_tf_coeff(9), estimated_tf_coeff(10), estimated_tf_coeff(11));
    ...
    TEG_info_extend_tab = array2table(TEG_info_extend, ...
        'VariableNames', ["Kn", "K0", "K1", "K2", "Kd", "Knc", "Kpc", "Kdc", "Kn2", "Kp2", "Kd2", "Rsq", "Fig Name"]);
