% Estimate TEG parameters (R, K, MA, etc.) using previous model. The
% parameter prediction methods is coded in
% TEG_Graph_Property_Identifier_6param.m, which is the same method used in
% "Quick model-based viscoelastic clot strength predictions from blood
% protein concentrations for cybermedical coagulation control"
% NOTE: The means and stds of the prediction error are not the same as
% those reported in the paper. This is because some entries correspond to
% experiments with errors. We manually filtered out those entires with
% errors. Please refer for the correct means and stds to the spreadsheets 
% "Prediction Errors of tPATXA dataset 9params vs 6params Method2.xlsx",
% "TEG_params_prediction_ACIT_6params_Method2.xlsx", and
% "TEG_params_prediction_ACIT_9params.xlsx".
...
TEG_info_filename = "TEG_Info_ACIT_6params_Method2.xlsx";
%% Result Exporting Settings
save_result = 0;            % Change to 0 if wish not to save the prediction table
export_filename = "TEG_params_prediction_ACIT_6params_Method2.xlsx";      % Must be a spreadsheet format
...
for sheetNo = 1:length(sheetNames)
    ...
    for i = 1:num_exp
        ...
        % end
        TEG_model_params_str = string(TEG_info(i,["Kn1", "Kp1", "Kd1", "Kn2", "Kp2", "Kd2"]).Variables);
        TEG_model_params_mat = zeros(6,1);
        ...
        simTEG = modelFit(TEG_model_params_mat, t);
        % figure
        % plot(t, simTEG, 'LineWidth', 2)
        % grid on
        % title(exp_names(i))
        ...
        [R_pred(i), K_pred(i), Ang_pred(i), MA_pred(i), TMA_pred(i), LY30_pred(i)] = TEG_Graph_Property_Identifier_6param(t, simTEG, 2);
    end
    RK_exp = R_exp + K_exp;
